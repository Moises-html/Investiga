...
const DialogueBox = (() => {
  ...
  function say(lines, { name = 'Moisés', onComplete = null, showCharacter = true } = {}) {
    queue = Array.isArray(lines) ? [...lines] : [lines];
    onDone = onComplete;
    nameEl.textContent = name;
    box.classList.add('visible');
    box.setAttribute('aria-hidden', 'false');
    if (showCharacter && !portrait) {
      portrait = Character.renderPortrait(box);
    }
    next();
  }
  ...
  function close() {
    ...
    if (portrait) {
      Character.removePortrait(portrait);
      portrait = null;
    }
  }
