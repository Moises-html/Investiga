...
const Character = (() => {
  ...
  /* Retrato estilo "visual novel": fica fixo colado à caixa de diálogo,
     de pé atrás dela, sempre que uma fala é exibida — não só a fala sozinha. */
  function renderPortrait(dialogueBoxEl) {
    const wrap = document.createElement('div');
    wrap.className = 'character character-portrait';
    ...
    function place() {
      const h = dialogueBoxEl.offsetHeight || 160;
      wrap.style.bottom = Math.max(h - 26, 40) + 'px';
    }
    place();
    window.addEventListener('resize', place);
    wrap._cleanupResize = () => window.removeEventListener('resize', place);

    requestAnimationFrame(() => requestAnimationFrame(() => wrap.classList.add('visible')));
    return wrap;
  }

  function removePortrait(wrap) {
    if (!wrap) return;
    if (wrap._cleanupResize) wrap._cleanupResize();
    remove(wrap);
  }

  return { render, remove, renderPortrait, removePortrait };
})();
